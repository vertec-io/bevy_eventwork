...
foreach ($file in $files) {
    ...
    # Remove the NetworkMessage import from eventwork_common if it exists
    $content = $content -replace 'use\s+eventwork_common::\{([^}]*),?\s*NetworkMessage\s*,?\s*([^}]*)\};', 'use eventwork_common::{$1$2};'
    $content = $content -replace 'use\s+eventwork_common::\{NetworkMessage\s*,?\s*([^}]*)\};', 'use eventwork_common::{$1};'
    $content = $content -replace 'use\s+eventwork_common::\{([^}]*),?\s*NetworkMessage\};', 'use eventwork_common::{$1};'
    $content = $content -replace 'use\s+eventwork_common::NetworkMessage;', ''
    
    # Clean up empty use statements
    $content = $content -replace 'use\s+eventwork_common::\{\s*\};', ''
