...
This example demonstrates how to build a **non-Bevy web client** that communicates with a bevy_eventwork WebSocket server using the `EventworkBincodeCodec`.

## Features

- ✅ **Leptos web framework** for reactive UI
- ✅ **leptos_use WebSocket** integration
- ✅ **EventworkBincodeCodec** for binary serialization (same as Bevy clients)
- ✅ **Real-time chat** with the eventwork server
- ✅ **Beautiful, responsive UI** with modern CSS

## Architecture

This example shows that bevy_eventwork servers can communicate with **any WebSocket client**, not just Bevy applications. The key is using the same binary codec (`EventworkBincodeCodec`) for message serialization.

```
┌─────────────────────┐         WebSocket + Bincode        ┌──────────────────────┐
│  Leptos Web Client  │ ◄──────────────────────────────► │  Bevy Server         │
│  (This Example)     │                                    │  (eventwork)         │
└─────────────────────┘                                    └──────────────────────┘
```
...
In one terminal, run the eventwork WebSocket server:

```bash
cd crates/eventwork_websockets
...
Both client and server use `EventworkBincodeCodec` which:
...
```rust
use_websocket_with_options::<NewChatMessage, UserChatMessage, EventworkBincodeCodec, _, _>(
    "ws://127.0.0.1:8081",
...
1. **Bevy WASM client**: `cd crates/eventwork_websockets/wasm_client_example && trunk serve`
2. **Leptos web client**: `cd crates/eventwork_websockets/leptos_client_example && trunk serve --port 8082`
3. **Bevy native client**: `cargo run --example client --package eventwork` (if you create a WebSocket version)
...
│   ├── codec.rs      # EventworkBincodeCodec wrapper
