...
This directory contains examples demonstrating how to use Bevy Eventwork.

## Examples

### automatic_messages.rs

**Demonstrates the new automatic message registration API** - the easiest way to use Eventwork!
...
**Run the server:**
```bash
cargo run --package eventwork --example server
```

**Run the client:**
```bash
cargo run --package eventwork --example client
```
...
Make sure you add the `EventworkPlugin` before registering messages:
```rust
