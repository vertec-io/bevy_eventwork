...
pub(crate) fn install<NP: NetworkProvider>(app: &mut App) {
    ...
    app.configure_sets(
            Update,
            (
                EventworkSyncSystems::Inbound,
                EventworkSyncSystems::Observe,
                EventworkSyncSystems::Outbound,
            )
                .chain(),
        )
        ...
        .add_systems(
            Update,
            handle_client_messages::<NP>.in_set(EventworkSyncSystems::Inbound),
        )
        // Connection lifecycle -> cleanup subscriptions and mutation queue
        .add_systems(
            Update,
            cleanup_disconnected.in_set(EventworkSyncSystems::Inbound),
        )
        // Process queued mutations: authorization + apply + MutationResponse
        .add_systems(
            Update,
            process_mutations::<NP>.in_set(EventworkSyncSystems::Inbound),
        )
...
pub fn process_snapshot_queue<NP: NetworkProvider>(world: &mut World) {
    ...
    info!(
        "[eventwork_sync] Processing {} snapshot batches for {} connections",
    ...
    if let Some(net) = world.get_resource::<Network<NP>>() {
        for (connection_id, items) in per_connection {
            ...
            info!(
                "[eventwork_sync] Sending snapshot batch: conn={:?}, items={}",
                connection_id,
                items.len()
            );
