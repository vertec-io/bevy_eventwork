...
//! Native client utilities for eventwork_sync (DEPRECATED).
//!
//! **⚠️ DEPRECATED**: This module is deprecated and will be removed in a future version.
//!
//! This module provides a non-reactive `SyncClient` for native Rust applications
//! (non-Leptos, non-WASM) that want to connect to an `eventwork_sync` server.
//!
//! ## Deprecation Notice
//!
//! This API is deprecated because:
//! - It's not currently used in any examples or production code
//! - The base `eventwork` crate provides better Bevy-to-Bevy networking
//! - Future versions will provide a dedicated Bevy-to-Bevy sync engine
//!
//! ## Migration Path
//!
//! - **For Bevy clients**: Use the base `eventwork` crate with `TcpProvider` or `WebSocketProvider`
//! - **For WASM/Leptos clients**: Use `eventwork_client::SyncProvider` and hooks
//! - **For future Bevy-to-Bevy sync**: Wait for the upcoming Bevy sync engine
//!
//! ## Current Use Cases
//!
//! This module is only useful if you need to:
//! - Connect a non-Bevy, non-Leptos Rust application to an `eventwork_sync` server
//! - Build a custom client that uses the `eventwork_sync` protocol
//!
...
/// Native client for eventwork_sync (DEPRECATED).
///
/// **⚠️ DEPRECATED**: Use base `eventwork` for Bevy-to-Bevy networking or
/// `eventwork_client::SyncProvider` for WASM/Leptos clients.
///
/// This provides a non-reactive API for native Rust applications to connect
/// to an `eventwork_sync` server. It is transport-agnostic - you provide a
/// `send` function that handles the actual transmission of messages.
///
/// # Example
///
/// ```rust,ignore
/// use eventwork_client::{NativeSyncClient, ClientTypeRegistry};
///
